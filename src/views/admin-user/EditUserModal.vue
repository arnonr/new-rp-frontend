    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title">แบบฟอร์มแก้ไขข้อมูลผู้ใช้งาน</h3>
          <button
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const fetchItems = async () => {
      ...
      const { data } = await ApiService.query(`user/${item.value.id}`, {});
      item_edit.value = {
        ...data.data,
      };
      item_edit.value.department_id = {
    ...
    const onSubmit = async () => {
      ...
      let data_item = {
        ...
        department_id: item_edit.value.department_id?.id,
        is_active: item_edit.value.is_active?.id,
      };

      await ApiService.put("user/" + item_edit.value.id, data_item)
        .then(({ data }) => {
          if (data.msg != "success") {
